...
my $dbh1 = DBI->connect('DBI:mysql:dbname=appdb', $db_user_name, $db_pw) || die "Could not connect to database: $DBI::errstr";
my $dbh2 = DBI->connect('DBI:mysql:dbname=bugs', $db_user_name, $db_pw) || die "Could not connect to database: $DBI::errstr";
...
while ($row = $sth1->fetchrow_arrayref()) {
    ...
    $once=0;
    #if ($i>=5) {last;}
    my $sth2 = $dbh2->prepare('SELECT bug_id, short_desc FROM bugs WHERE short_desc LIKE \'%' . $searchfor . '%\' AND bug_status != \'CLOSED\';');
    $sth2->execute();
